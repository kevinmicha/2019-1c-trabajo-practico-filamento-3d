...
RESET:	
		...
		rcall	ini_lcd			;inicializo el LCD
...
EVENTO1:
		...
		rcall	LEER_BITS
		ldi		ZL,		LOW(MSJ_DATO<<1)	; lectura de tensión
		ldi		ZH,		HIGH(MSJ_DATO<<1)
		rcall	enviar_frase
		rcall	delay_5seg
		rjmp	main
...
ESPERO_EN_BAJO_SCK:
		...
		LDI		R16,	IT_MEMORIA-1
		MOV		nro_veces,	R16
		STS		ADC_B,		R3
		STS		ADC_B+1,	R2
		STS		ADC_B+2,	R1
		LDI		YH,			HIGH(ADC_BUF)									;pongo puntero a ADC_BUF 
		LDI		YL,			LOW(ADC_BUF)
...
PROMEDIAR_MUESTRAS:
		...
		LDI		YH,			HIGH(ADC_B)									;pongo puntero en ADC_B que es primer byte de la suma
		LDI		YL,			LOW(ADC_B)
		LDI		R16,		8
